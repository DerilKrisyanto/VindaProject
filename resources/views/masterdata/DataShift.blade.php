...
<x-layout>
    <x-slot:title>Master Data Shift</x-slot:title>
    <div class="p-4">
        ...
        <!-- Input Shift Baru -->
        <form action="{{ route('SaveDataShift') }}" method="POST" class="mb-6">
            @csrf

            <!-- Input Nama Shift -->
            <label class="block font-medium mb-1">Nama Shift</label>
        ...
        <h3 class="text-lg font-semibold mb-2">Daftar Shift:</h3>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-900 uppercase tracking-wider">Nama Shift</th>
                            <th class="px-6 py-3 text-center text-xs font-semibold text-gray-900 uppercase tracking-wider">Aksi</th>
                        </tr>
                    </thead>

                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($dataShift as $index => $data)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-center font-semibold text-gray-900">{{ $index + 1 }}</td>
                                <td class="px-6 py-4 text-left text-gray-700">{{ $data->shift ?? '-' }}</td>
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('HapusDataShift', $data->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data ini?')">
                                        @csrf
                                        @method('DELETE')
